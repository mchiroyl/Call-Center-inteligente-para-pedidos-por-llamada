...
async def merge_async_iters(*aiters: AsyncIterator[T]) -> AsyncIterator[T]:
    ...
    async def producer(aiter: AsyncIterator[Any]) -> None:
        try:
            ...
            # Normal shutdown: the consumer stopped iterating early.
            pass
        except Exception:
            logger.debug("merge_async_iters producer error", exc_info=True)
        finally:
    ...
    try:
        ...
        tasks = [loop.create_task(producer(aiter)) for aiter in aiters]
        ...
        # Consumer closed the generator early — cancel all producers cleanly.
        for task in tasks:
            task.cancel()
        # Drain any remaining sentinels so tasks can exit.
        try:
